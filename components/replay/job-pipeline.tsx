import { AlertTriangle, CheckCircle2, CircleDashed, Clock3, RefreshCcw, XCircle } from "lucide-react";
...
function PipelineIcon({ status }: { status: PipelineStatus }) {
  ...
  if (status === "degraded" || status === "skipped") return <AlertTriangle className="h-4 w-4 text-saffron" />;
  if (status === "running") return <RefreshCcw className="h-4 w-4 animate-spin text-signal" />;
  if (status === "pending") return <CircleDashed className="h-4 w-4 text-muted" />;
